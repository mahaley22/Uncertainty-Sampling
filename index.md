...
We all know the aphorism: all models are wrong, but some are useful. Raw accuracy on seen or unseen data is just one metric of usefulness.   Besides, no matter how accurate (however imperfectly that might be measured), ML models can be subject to data drift, concept drift, or generalization issues and even outright bugs on unseen data.  Following software engineering best practices, how are things like unit testing, debugging, CI/CD and exception monitoring in production be accomplished with Deep Learning, which by definition is a nested non-linear structure optimized for accuracy, not scrutability.

Active Learning practice attempts to answer some of these questions: How do we optimize for humans in the loop?  How much hand-labeled training do you need up front and on an ongoing basis?  What types of errors can be more easily discovered for analysis?  Which subsets of outputs should humans be targetting for possible correction and training?  Active Learning has a lot of tools in the toolkit for sampling and iterating, both from data within the models and outside them.  This notebook looks at Attention-based  Encoder-Decoder MT model, and teases out several in-model metrics from the last layer (logits) of the decoder outputs.
